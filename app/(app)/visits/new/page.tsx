...
import { createVisitFromGlobalForm } from "@/app/(app)/visits/actions";
...
export default async function NewVisitPage({
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <div>
        <Link href="/visits" className="text-sm text-muted hover:text-foreground">
          ← Visits
        </Link>
        <h2 className="mt-2 text-xl font-semibold text-foreground">Record visit</h2>
        <p className="mt-1 text-sm text-muted">Choose a neighbor and log a home visit.</p>
      </div>
      ...
      <form
        action={createVisitFromGlobalForm}
      ...
      >
        ...
        <FormField label="Date" name="visit_date" type="date" required defaultValue={today} />
        <VolunteerSelect volunteers={volunteers} />
        <FormField label="Notes" name="notes" rows={4} />
        <FormField label="Next steps" name="next_steps" rows={2} />
